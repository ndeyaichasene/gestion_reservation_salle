...
<div class="page-header">
    <div>
        ...
        <p class="subtitle">Consultez et gérez les créneaux réservés sur le campus</p>
    </div>
    <a href="/reservations/create" class="btn btn-primary">+ Nouvelle réservation</a>
</div>

<div class="filter-bar">
    <form method="get" action="/reservations" style="display: flex; align-items: center; gap: 0.75rem; background: transparent; padding: 0; box-shadow: none;">
        <label for="salle_id">Filtrer par salle :</label>
        <select id="salle_id" name="salle_id" onchange="this.form.submit()">
            <option value="">-- Toutes les salles --</option>
            <?php foreach ($salles as $salle): ?>
                <option value="<?= $salle->id ?>" <?= $salleId === (int) $salle->id ? 'selected' : '' ?>>
                    <?= htmlspecialchars($salle->nom) ?> (<?= htmlspecialchars($salle->batiment) ?>)
                </option>
            <?php endforeach; ?>
        </select>
        <?php if ($salleId !== null): ?>
            <a href="/reservations" class="btn btn-secondary btn-sm">Réinitialiser</a>
        <?php endif; ?>
    </form>
</div>
        ...
        <tbody>
            ...
            <?php else: ?>
                ...
                <tr>
                    ...
                    <td>
                        <strong><?= htmlspecialchars($reservation->salle->nom ?? 'N/A') ?></strong>
                    </td>
                    ...
                    <td>
                        <a href="/reservations/<?= htmlspecialchars((string) $reservation->id) ?>" class="btn btn-secondary btn-sm">
                            Détails
                        </a>
                    </td>
                </tr>
